...
next_url = 'https://schedule.tau.ac.il/scilib/Web/schedule.php'
session = requests.session()
# header from post command
header = {"Content-Type": "application/x-www-form-urlencoded"}
payload = {'email': username, 'password': password, "resume": "", "login": "submit", "language": "en_gb"}
reply = session.post(url, cookies=session.cookies, headers=header, data=payload)
schedule_page = session.get(next_url)
...
soup1 = BeautifulSoup(schedule_page.text)
...
def delete_reservation(user_id,rn,token):
...
def find_open_slot(day,time,number_of_students):
        pass


def force_reservation(day,room,time,duration):
        pass
...
def scrape_users():
        ...
        reply = session.post(url, cookies=session.cookies, headers=header, data=payload)
        schedule_page = session.get(next_url)
        soupale = BeautifulSoup(schedule_page.text)
        res_id = str(soup.find("resid"))
        parameters_for_query = {"rid": desired_room_rid,  # change parameters
                                "sid": "1",
                                "rd": date,
                                "sd": sd,
